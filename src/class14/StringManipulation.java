package class14;

public class StringManipulation {

	public static void main(String[] args) {
		// 1 REPLACE

		String str = "I am a good tester#1!";
		String newStr = str.replace("a", "c");
		System.out.println(newStr);

		String newStr1 = str.replace("good", "great");
		System.out.println(newStr1);
		
		//what we are looking for to replace, new value

		String newStr2 = str.replace("tester", "programmer");
		System.out.println(newStr2);

		String newStr3 = str.replace("Tester", "programmer");
		System.out.println(newStr3);
		
		
		str=str.replace("!", "?");
		System.out.println(str);
		
		str=str.replace("1", "2");
		System.out.println(str);
		
		
		//2 REPLACE ALL -- will replace all that matches specified char
		
		String str1="12Hello 3234 World 465%^%";
		String replaced=str1.replaceAll("[0-9]", "");
		System.out.println(replaced);
		
		String newreplaced=replaced.replaceAll("[^A-Za-z]","");
		System.out.println(newreplaced);
		
		String replaceChar=replaced.replaceAll("[A-Za-z]", "");
		System.out.println(replaceChar);
		

		String str2="1Hello123";
		System.out.println(str2.replaceAll("[^0-9]",""));
		
		
	}

}
